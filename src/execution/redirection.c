...
/*   Updated: 2025/07/19 16:23:20 by zouazrou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	open_redirects(t_cmd *cmd)
{
	...
	while (cmd->redirects)
	{
		...
		else if (cmd->redirects->type == OUTPUT
			|| cmd->redirects->type == APPEND)
			cmd->fd_output = rd_output(cmd->redirects->filename, cmd->fd_output,
					cmd->redirects->type);
		if (cmd->fd_input == -1 || cmd->fd_output == -1
			|| cmd->redirects->type == AMBG)
		{
			ft_close(&cmd->fd_input);
			ft_close(&cmd->fd_output);
			cmd->error = true;
